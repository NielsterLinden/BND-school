...
> **Reading this brief after 18 Sep 2026.** Sections 1–8c are the brief of 16 Sep for the τhτh-only (v3) chapter;
> **§8d (the four-channel v4 rework) supersedes them** wherever they differ, and the delivered chapter is 5-02 … 5-33
> of `../03-sections-storyboard.md`. The v3-only clips (`tautau_g1/g2`, `tautau_h3`, `tautau_i1…i4`) and their data
> files `ztautau_sr_stack.json` / `ztautau_corrections.json` were removed; the stacks are read from `ztautau_v4.json`
> (data / simulation 2.89 → 1.01 with the fakes, not the v3 values 2.97 → 1.02 quoted below).
