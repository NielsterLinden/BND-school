...
def build_config(job: str, histo_file: str, meta: dict) -> str:
    ...
    edges = list(meta["bins"])
    drop = [i + 1 for i in range(len(edges) - 1) if edges[i + 1] <= config.SIDEBAND_REGION_MTT_MIN]   # 1-indexed bins
    for i, (region, label) in enumerate(zip(meta["regions"], meta.get("region_labels", meta["regions"]))):
        blocks.append(tc.region(region, Type="SIGNAL", HistoName=region, VariableTitle="m_{#tau#tau} [GeV]",
                                ...
                                DropBins=",".join(map(str, drop)) if i == 0 and drop else None))
    ...
    for name, info in meta.get("closure_nps", {}).items():
        ...
        if cat == "c0" and tag == "lo" and drop:
            continue                       # those bins are not fitted
