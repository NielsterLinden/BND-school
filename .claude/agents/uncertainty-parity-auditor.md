...
1. `.claude/CLAUDE.md` (repository), then the channel's `CLAUDE.md`, `handoff.md`, `README.md`, `docs/`.
2. `docs/UNCERTAINTY_PARITY.md` — **the procedure you execute**, its parity-file rules and pitfalls.
3. `docs/CONVENTIONS.md` (NP names, categories); the gaps already known per channel in
...
- Every shell: `source fitting/setup.sh` from the repository root (LCG_110; `trex-fitter` on PATH). Never
...
- The channel results were **frozen on 17 Sep 2026** (see the repository guide `.claude/CLAUDE.md`). Your run is a study next
...
## Steps (details in docs/UNCERTAINTY_PARITY.md)
