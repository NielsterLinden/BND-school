...
2. Read `presentation/CLAUDE.md`, `docs/06-chapter-anchors.md` (§A is fixed),
   `05-colour-schema.md`, `docs/07-manim-physics-recipes.md` §0, §5 (chaining), §6 (traps).
...
4. If the chain opens on a delivered clip, read that scene (`scenes/s4_zmumu.py` `ZmumuIntoDetector`,
...
- Never `-q h` (that is clip-deliverer's), never `source fitting/setup.sh` in this shell,
