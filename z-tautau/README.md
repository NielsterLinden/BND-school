...
shared `fitting/` conventions of all channels. This is the second iteration (v2): the first one was
reviewed in [`REVIEW.md`](REVIEW.md) and every issue raised there that could be fixed here was fixed
(section "What changed in v2" below).
...
## What changed in v2 (after the review)
...
| 3.2 fake factor without MC subtraction | MC-subtracted FF is nominal (`nosub` kept as a cross-check); W+jets subtracted with uniform weights (pathological raw weights) |
...
| BDT models | `/data/atlas/users/sjankovy/BND-school-cache/ztautau/bdt/` (recreated by step 3b in ~2 min) | 5 MB |
