...
- Read first: `presentation/CLAUDE.md`, then `presentation/docs/07-manim-physics-recipes.md` §0 and §6.
...
Cairo renderer only (never `--renderer=opengl`). Never `source fitting/setup.sh` here.
...
2. **Read the PNG frames** it writes to `work/<name>/`. Judge: clipping at the
   ...
   the top ~15 % (y > 2.7) free for the user's title, the top-left block
   (x < −5.85, y > 0.22) empty — the `keepout` line `render.py` prints must say ok.
