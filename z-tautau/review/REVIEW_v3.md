...
> Moved here from `z-tautau/` on 18 Sep 2026. Paths in this document are relative to `z-tautau/` (`docs/…`, `fit/…`, `review/…`).
...
`docs/CONVENTIONS.md` and z-mumu conventions this channel has to match. The ntuples in
...
* Subtracting simulated genuine-τ events from a fake-factor sideband is not an option in the method, it is
  ...
  then removes signal from μ. The written prompt (`.claude/prompts/z_tautau.md`) only asks for "the old time normal
...
* It carries the name `SigModel`, which `docs/CONVENTIONS.md` correlates across channels. In the
...
* `python review/fake_studies.py` (after `source ../fitting/setup.sh`): loads all ntuples in 6 s, reproduces the
