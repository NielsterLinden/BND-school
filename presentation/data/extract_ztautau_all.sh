...
#
#   cd <BND-school> && source fitting/setup.sh && bash presentation/data/extract_ztautau_all.sh [--check-only]
#
...
if ! python -c "import uproot, awkward" 2>/dev/null; then
    echo "extract_ztautau_all.sh: run 'source fitting/setup.sh' first (LCG env with uproot/awkward)" >&2
    exit 2
fi
MODE="${1:-}"
SEED="${SEED:-20260916}"
