...
tok = {
    ...
    "NDATA": f"{y['Data']['value']:,.0f}".replace(",", " "), "FAKEFRAC": f"{100*y['Fakes']['value']/y['Data']['value']:.0f}",
    "NDATAG": f"{r['cutflow']['data_2016G'].get('SR', 0):,}".replace(",", " ") if 'SR' in r['cutflow']['data_2016G'] else "@@NDATAG@@",
    "NDATAH": f"{r['cutflow']['data_2016H'].get('SR', 0):,}".replace(",", " ") if 'SR' in r['cutflow']['data_2016H'] else "@@NDATAH@@",
    "SB": f"{y['DYtautau']['value']/y['Fakes']['value']:.2f}", "NSST": f"~{r['genuine_tau_contamination']['SS_T']['data']:,}".replace(",", " "),
    "CONTAM": "| region | data | genuine τ1 (MC) | fraction |\n|---|---:|---:|---:|\n" + "\n".join(f"| {k} ({lab}) | {v['data']:,} | {v['mc_genuine_tau1']:,.0f} | {100*v['fraction']:.1f} % |".replace(",", " ") for k, lab, v in ((k, lab, r['genuine_tau_contamination'][k]) for k, lab in (("SS_T", "FF numerator"), ("SS_L", "FF denominator"), ("AR", "application"), ("OSAI_T", "C numerator")))),
    "COSSS": f"{r['fake_factors']['mcsub']['C_OS_SS_inclusive']:.3f}",
    "CCAT": ("| | 0 jets | 1 jet | ≥ 2 jets |\n|---|---:|---:|---:|\n" + "\n".join(f"| category {k}, Run2016{e} | " + " | ".join(f"{np.asarray(r['C_osss_per_category'])[ei, j, k]:.3f}" for j in range(3)) + " |" for k in range(3) for ei, e in enumerate("GH"))) if r.get("C_osss_per_category") else "@@CCAT@@",
...
prev_path = HERE / "review/filled_values.json"
...
for path in ["README.md", "handoff.md", "REVIEW.md", "docs/05-fake-factors.md", "docs/06-cross-section.md", "docs/07-corrections-and-systematics.md", "docs/08-fit-and-results.md", "docs/00-overview.md", "docs/02-selection.md", "slides/ztautau_slides.tex"]:
    ...
    for k, v in t.items():
        ...
        if refill and k in prev and prev[k] != v and len(str(prev[k])) >= 6 and prev[k] in s:
            s = s.replace(prev[k], v); n += 1
...
if "--dry-run" in sys.argv:
    ...
    prev_path.write_text(json.dumps({k: str(v) for k, v in tok.items()}, indent=1))
