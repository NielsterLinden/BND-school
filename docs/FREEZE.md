...
> **Repository cleanup, 18 Sep 2026 (after the talk was built).** This file is the record of the freeze and is kept as
> written, so a few paths below are historical: it moved here from the repository root; TRExFitter v1.8.0 is no longer
> a submodule (`fitting/build_trexfitter.sh` clones and builds it outside the repository); `setup.sh` is
> `fitting/setup.sh`; `fitting/CONVENTIONS.md` and `fitting/UNCERTAINTY_PARITY.md` are `docs/`; the repository guide is
> `.claude/CLAUDE.md`; `z-tautau/RESUME.md`, the git worktrees under `.claude/worktrees/` and the `00_archive/` clip
> folders no longer exist. The state before the cleanup is the tag `pre-cleanup-2026-09-18`; the frozen three-channel
> state is the tag `zmumu-freeze-2026-09-17`. **The final result is 1949 ⁺³⁰₋₃₀ pb** (second note below).
...
- `docs/CONVENTIONS.md`, `docs/UNCERTAINTY_PARITY.md`, `.claude/agents/uncertainty-parity-auditor.md`;
- the z-mumu docs;
- the presentation agent definitions in `.claude/agents/`.
...
`.claude/agents/` now quote the frozen numbers. Against the frozen outputs:
...
**Status, 17 Sep afternoon — section 4 (Z → μμ) is done** (built on branch `worktree-tnp-explainer`, merged into
`main` on 17 Sep): the three μμ extractors and `zmumu_{fit,sr_stack,tnp}.json` are on the frozen result (all checks pass; the
...
(4-01 … 4-21, `presentation/docs/briefs/zmumu.md`), and the brief, the storyboard and the
