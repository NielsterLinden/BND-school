...
> Run the uncertainty-parity procedure (`fitting/UNCERTAINTY_PARITY.md`) for **z-ee**. Reference
...
> repository `CLAUDE.md` (no barrel–endcap gap veto, placeholder ID SF there, `ElectronID` ±5.9 %). Measure what can be
...
> Run the uncertainty-parity procedure (`fitting/UNCERTAINTY_PARITY.md`) for **z-tautau**. Reference
...
> `z-tautau/output/results.json` (its `nominal_variant`), `z-tautau/fit/`, `z-tautau/REVIEW.md`,
