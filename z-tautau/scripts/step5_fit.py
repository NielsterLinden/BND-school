...
"""Step 5 -- binned profile-likelihood fit with TRExFitter v1.8.0 (docs/08-fit-and-results.md).
    ...
    python scripts/step5_fit.py [--ff-variant nominal|mcsub] [--asimov] [--skip-ranking]

Writes fit/ztautau[_mcsub].config (committed) from the histograms of step 4 using the shared
...
and parses everything with fitting/run_trex.py into fit/results/ztautau[_mcsub]_fit_result.json.
...
TITLES = {"DYtautau": "Z#rightarrow#tau#tau", "DYee": "Z#rightarrowee", "DYmumu": "Z#rightarrow#mu#mu",
          "DYlowmass": "Z/#gamma*#rightarrowll (m<50)",
          "WJets": "W+jets", "TTbar": "t#bar{t}", "SingleTop": "single t", "WW": "WW", "WZ": "WZ", "ZZ": "ZZ",
          "Fakes": "jet#rightarrow#tau_{h} (FF)"}
COLORS = {"DYtautau": 796, "DYee": 857, "DYmumu": 862, "DYlowmass": 795, "WJets": 632, "TTbar": 616, "SingleTop": 617,
...
def build_config(job: str, histo_file: str, meta: dict) -> str:
    ...
    blocks = [
        tc.job(job, Label="Z #rightarrow #tau_{h}#tau_{h}", CmeLabel="13 TeV", LumiLabel="16.4 fb^{-1}",
               ...
               ImageFormat="png", SystControlPlots=True, DoSummaryPlot=False, DoTables=True,
               DoSignalRegionsPlot=False, DoPieChartPlot=False, RankingMaxNP=20, RankingPlot="SYSTS",
               HistoChecks="NOCRASH", SystPruningShape=0.001, SystPruningNorm=0.001, GetChi2="TRUE",
               SystCategoryTables=True, RatioYmax=1.5, RatioYmin=0.5, POIPrecision=3),
        tc.fit("fit", FitType="SPLUSB", FitRegion="CRSR", UseMinos="mu_Z", NumCPU=4),
        tc.region(config.REGION, Type="SIGNAL", HistoName=config.REGION, VariableTitle="m_{#tau#tau} [GeV]",
                  Label="#tau_{h}#tau_{h} SR", ShortLabel="SR"),
    ...
    for s in ("DYlowmass", "WJets", "TTbar", "SingleTop", "WW", "WZ", "ZZ"):
    ...
    # signal modelling (acceptance-normalised: only the C factor varies)
    for s, t in (("QCDScale", "QCD scales"), ("PDF", "PDF"), ("PS_ISR", "PS ISR"), ("PS_FSR", "PS FSR")):
        blocks.append(tc.histo_syst(s, samples.SIGNAL, "Signal modelling", title=t))
    if meta.get("has_sigmodel"):
        # normalisation (C factor) only: the LO shape is not a credible alternative to the NLO one and the
        # data would otherwise constrain this one-sided variation through the shape
        blocks.append(tc.block("Systematic", "SigModel", Title="generator (LO vs NLO)", Type="HISTO",
                               HistoNameSufUp="__SigModelUp", Samples=samples.SIGNAL, Symmetrisation="ONESIDED",
                               DropShapeIn="all", Category="Signal modelling"))
    # fakes
    for dm in config.TAU_DMS:
        blocks.append(tc.histo_syst(f"FakeStat_tautau_DM{dm}", "Fakes", "Fakes", title=f"FF stat. DM{dm}"))
    blocks.append(tc.histo_syst("FakeOSSS_tautau", "Fakes", "Fakes", title="FF OS/SS extrapolation"))
    blocks.append(tc.histo_syst("FakeClosure_tautau", "Fakes", "Fakes", title="FF non-closure"))
    header = (f"Z -> tau_h tau_h, generated by z-tautau/scripts/step5_fit.py (do not edit by hand)\n"
              f"fake-factor variant: {meta['variant']}; fit variable {meta['fit_variable']}; TRExFitter v1.8.0")
...
def main():
    ...
    args = ap.parse_args()
    nominal_variant = "mcsub" if config.FF_SUBTRACT_MC else "nominal"
