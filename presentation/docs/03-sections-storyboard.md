# 03 — Sections and clips (final, as delivered for the talk of 18 Sep 2026)

Seven sections; one clip = one idea; **clip number = play order** within a section. The tables list exactly the
99 clips in `clips/` (= `clips/CLIPLIST.tsv`), each with what moves and the source of what it shows. Earlier cuts,
retired clips and superseded versions were removed from the repository on 18 Sep 2026; they are in the git history
(tag `pre-cleanup-2026-09-18`).
...
numbers `data/theory_reference.json`. The Drell-Yan diagram itself lives in `scenes/s1_drell_yan.py` (the base class of
`TheoryProcess` and of the channel diagrams): 1-02 ends on its frame.
...
`scenes/s2_pipeline.py` (the earlier one-row pipeline) is kept as a library only: the map imports its icons, camera
and `masked_play` from it.
...
(z-ee delivery 16 Sep, `docs/FREEZE.md`). Brief `briefs/zee.md`; scene `scenes/s3_zee_story.py` (`ZeeSelection ZeeFit`);
...
Final pass of 17 Sep 2026 on the frozen result (`docs/FREEZE.md`, tag `zmumu-freeze-2026-09-17`); brief:
...
The fake factor is left to Z→ττ. Why tag-and-probe first and in full: the largest single data/MC correction (prediction −2.4 %), measured by us
...
The four-channel measurement (τhτh + μτh + eτh + eμ, z-tautau v4): the τhτh story up to the BDT categories, then the
lepton channels (`TautauLepHad`, `TautauEmu`) and the combined fit (`TautauFit`), result 1981 +153 −136 pb. Brief
`briefs/ztautau.md`; scenes `scenes/s5_ztautau.py` (5-02 … 5-04) and `scenes/s5_ztautau_story.py`; numbers
`data/ztautau_*.json`. **Clip number = play order.**

| # | clip | what moves | source |
|---|-----------|-----------|--------|
| ✔ 5-02 | `tau_decay` | τ⁻ → ν_τ W*⁻, W*⁻ → d ū, hadronisation blob, π⁻ π⁰ (decay mode 1); the only neutrino is the ν_τ | z-tautau/docs/02-selection.md |
| ✔ 5-03 | `tau_jet` | chained: the boosted τ: pions collimate into a narrow cone, π⁰ → γγ | — |
| ✔ 5-04 | `tautau_detector` | chained: dissolves to the slice; a 1-prong τ_h (track, γγ in the ECAL, HCAL) and a 3-prong τ_h, two ν_τ leave unseen, the p_T^miss arrow recoils; camera zooms in (centre (−0.2, −0.25): the chapter-identifier corner stays white) | `signature("tau_h", prongs=...)`, `"met"` |
| ✔ 5-05 | `tautau_a1_event` | chained on 5-03 (**real data from here on**, `data/ztautau_*.json`; brief `briefs/ztautau.md`): camera zooms out, the schematic τ_h / ν / MET fade, a real SR event (τ_h tracks from φ, charge, DM, p_T; the real p_T^miss arrow), p_T values | data/ztautau_events.json |
| ✔ 5-06 | `tautau_a2_visible_mass` | chained: the p_T values converge to m_vis, below the dashed m_Z | data/ztautau_events.json |
| ✔ 5-07 | `tautau_b1_neutrinos` | chained: dashed ν arrows along each τ_h; their vector sum lands on the p_T^miss arrow | docs/04-ditau-mass.md |
| ✔ 5-08 | `tautau_b2_likelihood` | chained: the (x₁, x₂) posterior grid of the event lights up; m_ττ = m_vis/√(x₁x₂), the median cell; m_ττ replaces m_vis | data/ztautau_events.json |
| ✔ 5-09 | `tautau_b3_shapes` | chained: the Z→ττ signal simulation only (inclusive sample, one peak): m_vis (grey ghost, 0.80 m_Z) vs m_ττ (red, 0.99 m_Z); 13 % → 11 % | data/ztautau_mass.json, docs/04 |
| ✔ 5-10 | `tautau_c1_first_entry` | chained: slice parks left, linear m_ττ axes (14 fit bins, 0–350 GeV), first entry | data/ztautau_v4.json |
| ✔ 5-11 | `tautau_c2_rain` | chained: clock, rain, the 14 data bins grow; N = 21,160 | data/ztautau_v4.json |
| ✔ 5-12 | `tautau_d1_simulation` | chained: bars → points; the simulation stack (rest / non-fid Z→ττ / fid Z→ττ) reaches a third of the data; key | data/ztautau_v4.json |
| ✔ 5-13 | `tautau_d2_gap` | chained: ratio panel opens at data / simulation = 2.89 | data/ztautau_v4.json |
| ✔ 5-14 | `tautau_e1_same_sign` | chained: plot parks; the slice in the dashed τ_h^±τ_h^± box with a real same-sign pair (τ₁ a jet) | data/ztautau_events.json, docs/05 |
| ✔ 5-15 | `tautau_e2_tally` | chained: ten real same-sign pairs, 2 of 10 pass → f = N_T/N_L = 2/8 | data/ztautau_events.json |
| ✔ 5-16 | `tautau_e3_ff_map` | chained: the real 4 × 5 fake-factor map (period G, 0 jets), rows 1-prong / 1-prong+π⁰ / 3-prong / 3-prong+π⁰; f = f(period, DM, N_jets, p_T) written, no more tables | data/ztautau_fakes.json |
| ✔ 5-17 | `tautau_e4_eta_closure` | chained: first the same-sign data vs FF prediction in \|η(τ₁)\| with its ratio (0.93 … 1.14 … 0.84), then the ratio becomes f(\|η\|) and slides to 1; the same for p_T(τ₂) | data/ztautau_fakes.json, docs/05 |
| ✔ 5-18 | `tautau_e5_osss` | chained: C_OS/SS = C(period, N_jets, D_BDT) ∈ [0.96; 1.305] (no tables); the box turns opposite-sign | data/ztautau_fakes.json |
| ✔ 5-19 | `tautau_f1_apply` | chained: N_fake = C_OS/SS · f × N_AR; the plot returns | docs/05 |
| ✔ 5-20 | `tautau_f2_template` | chained: the fake template fills the gap; ratio 2.89 → 1.01; N_fake = 13592 (64 %) | data/ztautau_fakes.json |
| ✔ 5-21 | `tautau_h1_inputs` | own scene from a blank frame: the 16 BDT input symbols by importance (no mass variable) | data/ztautau_bdt_inputs.json |
| ✔ 5-22 | `tautau_h2_shapes` | chained: unit-normalised fakes (pale slate) vs Z→ττ (red) distributions of the leading inputs, one panel after the other | data/ztautau_bdt_inputs.json |
| ✔ 5-23 | `tautau_h4_score` | chained: the real D_BDT output (log y from 10^1.5, ratio panel): fakes, Z→ττ, non-fid, rest, data; cuts at 0.55 / 0.90; AUC 0.966 | data/ztautau_bdt.json |
| ✔ 5-24 | `tautau_h5_categories` | chained: the plot splits into SR0 / SR1 / SR2 panels; SR0 < 110 GeV greyed; 486/12351, 1131/1044, 2141/197 | data/ztautau_bdt.json, data/ztautau_fit.json |
| ✔ 5-25 | `tautau_k1_lephad_fakes` | μτ_h: the application region is multijet / W+jets / tt̄ (fractions); each process has its own fake factor from its own region; N_fake = 11,048 | data/ztautau_v4.json, z-tautau/docs/10 |
| ✔ 5-26 | `tautau_k2_w_charge` | chained: the W+jets fake factor depends on the charge: opposite-sign quark jet vs same-sign gluon jet (schematic cones) | z-tautau/docs/10 |
| ✔ 5-27 | `tautau_k3_lephad_closure` | chained: the same-sign validation, prediction vs data: 0.966 ± 0.014 | data/ztautau_v4.json |
| ✔ 5-28 | `tautau_l1_emu_regions` | eμ signal region (D_ζ > −20, no b) and tt̄ control region (D_ζ < −40, p_T^miss > 80) side by side, prefit | data/ztautau_v4.json |
| ✔ 5-29 | `tautau_l2_emu_ttbar` | chained: μ_tt̄ floats, slider 1 → 1.11 ± 0.04; both panels go post-fit | data/ztautau_v4.json |
| ✔ 5-30 | `tautau_m1_sf_lever` | the lever arm of the τ_h ID scale factor: eμ ∝ μ_Z, ℓτ_h ∝ SF·μ_Z, τ_hτ_h ∝ SF²·μ_Z | z-tautau/docs/10 |
| ✔ 5-31 | `tautau_m2_sf_fit` | chained: the τ_h ID SF per decay mode, TauPOG (grey) vs in situ (red) | data/ztautau_v4.json |
| ✔ 5-32 | `tautau_m3_postfit` | chained: the four channels post-fit with data / pred.; μ_Z = 1.019 +0.079 −0.070 (with the τ_h ID SF p_T dependence `TauIDpT_tautau`) | data/ztautau_v4.json |
| ✔ 5-33 | `tautau_m4_sigma` | chained: σ(60–120) = 1981 +153 −136 pb, the four channels alone as open points, the prediction with its band (CMS and ATLAS are left for the end of the talk) | data/ztautau_v4.json, data/ztautau_reference.json |
