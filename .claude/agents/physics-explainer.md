...
- **Read-only grounding**: `z-mumu/docs/*.md`, the four `*/handoff.md`,
  `docs/CONVENTIONS.md`, `datasets/`, `docs/OpenDataIntro.pdf`,
  `presentation/docs/03-sections-storyboard.md`.
...
- Detector drawings: `CMSSlice` + `signature(det, kind, phi)` from the style module
  (kinds e, mu, gamma, tau_h, jet, nu); see `scenes/s4_zmumu.py` (`ZmumuIntoDetector`) for the pattern.
