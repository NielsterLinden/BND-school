...
1. `.claude/CLAUDE.md` (repository), `presentation/CLAUDE.md`
2. `presentation/docs/06-chapter-anchors.md` (fixed / default / ideas),
...
5. The worked example `presentation/docs/briefs/zmumu.md`, and skim the
...
Copy `presentation/docs/briefs/TEMPLATE.md` to
`presentation/docs/briefs/<chapter>.md` (`zee`, `ztautau`, …) and fill every
...
- Renders are serial (`flock work/render.lock …`). Environment for every Manim shell:
  ...
  never `source fitting/setup.sh` in that shell (the LCG env is only for extraction).
