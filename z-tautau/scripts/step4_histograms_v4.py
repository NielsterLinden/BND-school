...
"""Step 4 (v4) -- fit inputs of the four-channel fit (docs/10-v4-plan.md sections 8-9).
    ...
    python scripts/step4_histograms_v4.py [--channels tautau mutau etau emu] [--no-plots]
...
BINS.update({r: config.FIT_BINS_LTAU for r in config.LTAU_REGIONS})
...
SYST_INFO = {   # name -> (category, kind, subcategory, smoothing, up, down, title)
    ...
    "ElectronID": ("Electron efficiency", "HISTO", None, None, None, None, "electron ID"), "ElectronTrigger": ("Electron efficiency", "HISTO", None, None, None, None, "Ele27 trigger (in situ)"),
    "EmuTrigger": ("Electron efficiency", "HISTO", None, None, None, None, "e#mu cross trigger (in situ)"), "BTag": ("b tagging", "HISTO", None, None, None, None, "b-tag efficiency"),
...
def dm_regions(channel):
    """[(region name, decay mode or None)] of a lepton channel (one region per tau_h decay mode, docs/10 section 8)."""
    if config.LTAU_DM_REGIONS:
        return [(f"{channel}_SR_dm{dm}", dm) for dm in config.TAU_DMS]
    return [(f"{channel}_SR", None)]
...
def ltau(book, registry, yields, channel, ctrl, plots):
    ...
    def dm_mask(d, dm):
        return np.ones(len(d["run"]), bool) if dm is None else (d["t_dm"] == dm)
    ...
    for region, dm in regs:
        sr, ar = reg["SR"] & dm_mask(data, dm), reg["AR"] & dm_mask(data, dm)
    ...
    for v, name in fake_vars.items():
        registry.syst(name, ["Fakes"], reg_names, "Fakes", title={"osss": "multijet FF OS/SS", "wmt": "W FF m_{T} extrapolation", "frac": "AR fractions (W #pm 20%)", "tt": "t#bar{t} FF (30%)"}[v] + f" {channel}")
    for v, (ed, _) in ctrl_vars.items():
    ...
    for key in an.available_mc(channel):
        ...
        masks = {region: r["SR"] & dm_mask(d, dm) for region, dm in regs}
        ...
        def kin_fn(syst, d_, d=d, key=key, cand=cand):
            ...
            return kin, {region: sr & dm_mask(d, dm) for region, dm in regs}
...
def main():
    ...
    meta = {"regions": regions, "bins": {r: list(BINS[r]) for r in regions}, "lumi_pb": config.LUMI_PB, "channels": args.channels,
