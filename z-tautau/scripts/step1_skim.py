...
def main():
    ...
    if args.one_file:                                   # worker mode
        skim.process_file(args.one_file, args.out, samples.SAMPLES[args.sample])
        return
    ...
    if not args.manifest_only:
        for key in keys:
            ...
            print(f"[skim] {key}: {len(tasks)} files -> {config.SKIM_DIR / key}", flush=True)
            t0 = time.time()
            notes = batch.run_files(tasks, Path(__file__), config.SKIM_DIR / key, args.workers,
    ...
    manifest = skim.build_manifest([k for k in keys if skim.skim_files(k)])
