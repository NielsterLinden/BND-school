...
def prepare(manifest, *, run_dir=None, acceptance_factors=None, preliminary=False):
    ...
    if set(acceptance_factors) != {"mumu", "ee", "tautau"}:
        raise ValueError("Acceptance factors must specify all three channels")
    ...
    if set(manifest['channels']) != {'mumu', 'ee', 'tautau'}:
        raise ValueError('Exactly mumu, ee and tautau are required; no silent channel omission')
    ...
    multi = [['MultiFit', 'combLieke', dict(
        ...
        Label='Z to ll: mumu + ee + tautau', CmeLabel='13 TeV',
        LumiLabel='16.4 fb^{-1}', NumCPU='4', NLLOffset='INITIAL', FitStrategy='2', ToleranceScale='10')]]
    for channel, spec in manifest['channels'].items():
        ...
        for kind, name, opts in blocks:
            ...
            if kind == 'Fit':
                opts.update(UseMinos='mu_comb', NLLOffset='INITIAL', FitStrategy='2', ToleranceScale='10', ErrorDecomposition='FALSE')
...
def main():
    ...
    try:
        ...
        print('Prepared three channel configs and generated/multifit.config')
        if args.preliminary:
